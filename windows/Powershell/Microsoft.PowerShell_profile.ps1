...
function proj { cd D:\Documents\Projects\ }
function d { cd D:\ }
function downloads { cd D:\Downloads}
function cdownloads { cd C:\Users\macke\Downloads}
function software { cd D:\Documents\Software\ }
...
function gitmergecode { git status --porcelain | Select-String -Pattern "UU" | ForEach-Object { $_ -replace "UU ", "" } | ForEach-Object { code $_ } }
function gitcsprojcode { git status --porcelain | Select-String -Pattern " M.*csproj" | ForEach-Object { $_ -replace " M ", "" } | ForEach-Object { code $_ } }
...
function gc! { & git commit -ev --amend $args }
function gclean { & git clean -fX }
function gcleanvs { & git clean -fX -e !.vs }
function gco { & git checkout $args }
function gcob { & git checkout -b $args }
function gcm { & git checkout master $args }
function gcl { & git checkout - $args }
...
function gdss { & git diff --shortstat $args }
function gf { & git fetch $args }
function gfa { & git fetch --all --prune $args }
function gfo { & git fetch origin $args }
function gg { & git grep -n $args }
function ggwt([Parameter(Mandatory=$true)][string]$Pattern, [string]$filePattern="*.cs") { & git grep -n $Pattern -- $filePattern ":!*Fixture*" ":!*Test*" }
